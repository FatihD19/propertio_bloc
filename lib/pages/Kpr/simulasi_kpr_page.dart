...
import 'package:propertio_bloc/shared/ui/components/textfieldForm.dart';
import 'package:propertio_bloc/shared/ui/widgets/item_angsuran.dart';
...
class _SimulasiKprPageState extends State<SimulasiKprPage> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget listAngsuran() {
      return Column(
        children: [
          BlocConsumer<KprCubit, KprState>(
            listener: (context, state) {
              if (state is KprError) {
                showMessageModal(context, state.message);
              }
            },
            builder: (context, state) {
              return BlocBuilder<KprCubit, KprState>(
                builder: (context, state) {
                  if (state is KprError) {
                  ...
                  if (state is KprLoaded) {
                    return Center(
                      child: Column(
                        children: [
                          ringkasan(
                              formatCurrencyDouble(state
                                  .installmentResults.summaryPrincipalLoan!),
                              formatCurrencyDouble(state
                                  .installmentResults.summaryInterestPrice!),
                          ...
                          Wrap(
                              ...
                              runSpacing: 8,
                              children: state
                                  .installmentResults.installmentByYear!
                                  .map((e) => ItemAngsuran(
                                      formatCurrencyDouble(e.installment),
                                      e.year.toString(),
                                      isSelected: loanTermController.text ==
